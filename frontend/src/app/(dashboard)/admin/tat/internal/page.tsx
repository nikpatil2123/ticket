...
export default function InternalTATPage() {
  ...
  const fetchSettings = async () => {
    try {
      setIsLoading(true);
      const res = await apiClient.get('/settings/tat_internal');
      if (res.data?.data) {
        setSettings(res.data.data);
      }
    } catch (err) {
      console.error('Failed to load Internal TAT settings', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await apiClient.put('/settings/tat_internal', settings);
      alert('Internal TAT Rules saved successfully!');
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Configure SLA rules for internal stakeholders (faculty, staff).</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="mt-8">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Internal TAT Tickets</h3>
        <div className="h-[600px]">
          <TicketList tatType="INTERNAL" />
        </div>
      </div>
    </div>
  );
